...
function SettingsContent() {
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {/* 2-Column Settings Grid: Account (Left) & Connected Apps (Right) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        {/* CARD 1: Account (Profile + Appearance + Session & Sign Out) */}
        <div className="rounded-2xl bg-surface border border-border-subtle p-4 md:p-6 shadow-xs flex flex-col space-y-4 md:space-y-5">
          {/* Profile Header */}
          <div className="flex items-center justify-between pb-3.5 md:pb-4 border-b border-border-subtle/70 gap-2">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-purple-500/15 text-purple-600 dark:text-purple-400 flex items-center justify-center text-sm md:text-base font-extrabold uppercase shrink-0">
                {username?.charAt(0) || 'U'}
              </div>
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 min-h-[44px] md:min-h-0 md:h-8 px-3.5 py-1.5 rounded-xl bg-surface hover:bg-surface-elevated text-text-primary text-xs font-semibold border border-border-subtle transition-colors shadow-2xs shrink-0 cursor-pointer"
            >
          ...
          {/* Mobile-Optimized Grouped Metadata List (< md) */}
          <div className="divide-y divide-border-subtle/40 pt-1 md:hidden">
            ...
            <div className="flex items-center justify-between py-2.5 text-xs">
              <span className="text-text-secondary font-medium">Account Security</span>
          ...
          {/* Desktop 3-Column Metadata Row (>= md) */}
          <div className="hidden md:block">
            ...
            <div className="flex items-center justify-between pt-3.5 mt-3.5 border-t border-border-subtle/50 text-xs text-text-secondary">
          ...
          <div className="pt-3 md:pt-4 border-t border-border-subtle/70">
            <div className="flex items-center justify-between mb-2 md:mb-2.5">
              <div className="flex items-center gap-2 text-text-primary">
                <Sun className="w-4 h-4 text-primary" />
                <h3 className="text-sm font-bold leading-tight">Appearance</h3>
              </div>
              <span className="text-[11px] text-text-secondary hidden sm:inline">
                Customize how Memeboard looks
              </span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-2.5">
              ...
              <button
                ...
                className={`flex items-center justify-between min-h-[44px] p-2.5 md:p-3 rounded-xl border transition-all cursor-pointer ${
                  currentTheme === 'light'
              ...
              >
                <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                  ...
                  <span className="text-xs md:text-sm font-medium truncate">Light</span>
                </div>
                <div
              ...
              >
                <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                  ...
                  <span className="text-xs md:text-sm font-medium truncate">Dark</span>
                </div>
                <div
                  className={`w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full border flex items-center justify-center shrink-0 ${
                    currentTheme === 'dark'
              ...
              >
                <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                  ...
                  <span className="text-xs md:text-sm font-medium truncate">System</span>
                </div>
                <div
                  className={`w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full border flex items-center justify-center shrink-0 ${
                    currentTheme === 'system'
          ...
          {/* Compact Session & Sign Out Section */}
          <div className="pt-3 md:pt-4 border-t border-border-subtle/70">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 md:p-3.5 rounded-xl bg-surface-elevated/40 border border-border-subtle/60">
              <div className="min-w-0">
                <div className="flex items-center gap-1.5 text-text-primary">
                  <Shield className="w-3.5 h-3.5 text-text-secondary" />
                  <span className="text-xs font-bold leading-tight">Session Security</span>
                </div>
                <p className="text-[11px] text-text-secondary mt-0.5 leading-relaxed">
                  Signing out ends your current session on this browser.
                </p>
              </div>
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 min-h-[44px] sm:min-h-0 sm:h-9 px-4 rounded-xl border border-red-500/25 bg-red-500/[0.04] hover:bg-red-500/10 text-red-600 dark:text-red-400 text-xs font-bold transition-all shadow-2xs shrink-0 cursor-pointer active:scale-[0.98]"
              >
        ...
        <div className="rounded-2xl bg-surface border border-border-subtle p-4 md:p-6 shadow-xs flex flex-col space-y-4">
          <div>
          ...
          <div className="space-y-3.5">
            {/* Telegram Bot Row */}
            <div className="p-3.5 rounded-xl bg-surface-elevated/60 border border-border-subtle/70 flex flex-col justify-between gap-2.5">
              {/* Top: Identity & Status */}
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-[#229ED9]/15 text-[#229ED9] flex items-center justify-center shrink-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <a
                        ...
                        className="text-xs text-[#229ED9] hover:underline inline-flex items-center gap-0.5 font-medium truncate"
                      >
                ...
                {/* Status Badge */}
                {isTelegramConnected ? (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-semibold border border-emerald-500/20 shrink-0">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="truncate max-w-[100px]">{telegramUsername ? `@${telegramUsername}` : 'Connected'}</span>
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-surface text-text-secondary text-xs font-medium border border-border-subtle/80 shrink-0">
                ...
                {isTelegramConnected ? (
                  <>
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 inline-flex items-center justify-center gap-1.5 min-h-[44px] sm:min-h-0 sm:h-10 px-3 rounded-xl bg-[#229ED9] hover:bg-[#229ED9]/90 text-white text-xs font-bold transition-all shadow-xs"
                    >
                    ...
                    <button
                      ...
                      className="min-h-[44px] sm:min-h-0 sm:h-10 px-3 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 text-xs font-bold transition-all border border-rose-500/20 disabled:opacity-50 inline-flex items-center gap-1 shrink-0 cursor-pointer"
                  ...
                  <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-1.5 min-h-[44px] sm:min-h-0 sm:h-10 px-4 rounded-xl bg-[#229ED9] hover:bg-[#229ED9]/90 active:scale-[0.99] text-white text-xs font-bold shadow-xs transition-all disabled:opacity-75 cursor-pointer"
                  >
                    {isGeneratingCode ? (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    ) : (
            ...
            <div className="p-3.5 rounded-xl bg-surface-elevated/60 border border-border-subtle/70 flex flex-col justify-between gap-2.5">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-[#25D366]/15 text-[#25D366] flex items-center justify-center shrink-0">
                    <svg className="w-4 h-4 md:w-5 md:h-5 fill-current" viewBox="0 0 24 24">
              ...
              <div className="pt-1">
                <button
                  type="button"
                  disabled
                  className="w-full inline-flex items-center justify-center gap-1.5 min-h-[44px] sm:min-h-0 sm:h-10 px-3.5 rounded-xl bg-surface-elevated text-text-secondary/60 border border-border-subtle/60 text-xs font-semibold cursor-not-allowed"
                >
          ...
          {!isTelegramConnected && (
            <div className="pt-1">
              {linkCode ? (
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    <button
                      ...
                      className="text-text-secondary hover:text-text-primary underline flex items-center gap-1 font-medium min-h-[44px] sm:min-h-0 py-1"
                    >
                    ...
                    <button
                      ...
                      disabled={isGeneratingCode}
                      className="text-text-secondary hover:text-primary flex items-center gap-1 font-medium transition-colors disabled:opacity-50 text-[11px] min-h-[44px] sm:min-h-0 py-1"
                    ...
                    <div className="mt-2 p-3 rounded-xl bg-surface border border-border-subtle flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="min-h-[44px] sm:min-h-0 sm:h-8 px-3 rounded-lg bg-surface-elevated hover:bg-black/5 dark:hover:bg-white/10 text-text-secondary hover:text-text-primary text-xs font-semibold transition-all flex items-center gap-1 border border-border-subtle cursor-pointer"
                        >
                          {copied ? (
                            <>
                ...
                <div className="p-3 rounded-xl bg-surface-elevated/50 border border-border-subtle flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    onClick={handleGenerateNewCode}
                    disabled={isGeneratingCode}
                    className="inline-flex items-center justify-center gap-1.5 min-h-[44px] sm:min-h-0 sm:h-8 px-3 py-1.5 rounded-xl bg-[#229ED9]/10 hover:bg-[#229ED9]/20 text-[#229ED9] text-xs font-bold transition-all border border-[#229ED9]/20 disabled:opacity-50 shrink-0 cursor-pointer"
                  >
                    {isGeneratingCode ? (
